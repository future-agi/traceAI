...
requests and responses for FI tracing. It handles message parsing, token
...
def _get_input_messages(content: Any) -> List[Message]:
    """
    Convert raw message content to FI Message objects.
...
def get_llm_input_attributes(request_body: Dict[str, Any], model_id: str) -> Dict[str, Any]:
    ...
    set of attributes for FI tracing, including model details,
    ...
    return {
        **get_llm_attributes(
            ...
            system=FiLLMSystemValues.ANTHROPIC.value,
            provider=FiLLMProviderValues.AWS.value,
            invocation_parameters=invocation_parameters,
            input_messages=input_messages,
        ),
        **get_span_kind_attributes(FiSpanKindValues.LLM),
...
def get_llm_output_attributes(response_body: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Processes the response body to create attributes for FI tracing,
