"""Tests for Fireworks wrapper functions."""

import pytest
from unittest.mock import MagicMock, PropertyMock

from traceai_fireworks._wrappers import (
    _is_fireworks_client,
...
class TestIsFireworksClient:
    """Tests for _is_fireworks_client function."""

    def test_returns_true_for_fireworks_client(self, mock_fireworks_client):
        """Should return True for client with Fireworks base URL."""
        assert _is_fireworks_client(mock_fireworks_client) is True

    def test_returns_false_for_non_fireworks_client(self, mock_non_fireworks_client):
        """Should return False for client with non-Fireworks base URL."""
        assert _is_fireworks_client(mock_non_fireworks_client) is False

    def test_returns_false_for_no_client(self):
        """Should return False when instance has no _client."""
        instance = MagicMock(spec=[])
        assert _is_fireworks_client(instance) is False
...
class TestGetInputAttributes:
    ...
    def test_basic_attributes(self):
        ...
        assert attrs[SpanAttributes.GEN_AI_PROVIDER_NAME] == "fireworks"
        assert attrs[SpanAttributes.GEN_AI_PROVIDER_NAME] == "fireworks"
        assert attrs[SpanAttributes.GEN_AI_REQUEST_MODEL] == "accounts/fireworks/models/llama-v3p1-8b-instruct"
    ...
    def test_handles_none_values(self):
        ...
        # No invocation params should be set when all values are None
