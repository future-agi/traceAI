"""Tests for Fireworks instrumentor."""

import pytest
from unittest.mock import MagicMock, patch, PropertyMock
import sys

from traceai_fireworks import FireworksInstrumentor, __version__


class TestFireworksInstrumentor:
    """Tests for FireworksInstrumentor class."""
    ...
    def test_instrumentation_dependencies(self):
        ...
        instrumentor = FireworksInstrumentor()
    ...
    def test_handles_missing_openai(self):
        ...
        with patch.dict(sys.modules, {"openai": None}):
            with patch("traceai_fireworks.import_module", side_effect=ImportError):
                instrumentor = FireworksInstrumentor()
                # Should not raise
                try:
                    instrumentor.instrument(tracer_provider=mock_provider)
...
class TestFireworksInstrumentorIntegration:
    """Integration tests for FireworksInstrumentor."""

    @patch("opentelemetry.exporter.otlp.proto.http.trace_exporter.OTLPSpanExporter")
    def test_wrapper_skips_non_fireworks_client(self, mock_exporter):
        """Should skip instrumentation for non-Fireworks clients."""
        from traceai_fireworks._wrappers import _CompletionsWrapper

        mock_tracer = MagicMock()
        wrapper = _CompletionsWrapper(tracer=mock_tracer)

        # Create non-Fireworks client
    ...
    @patch("opentelemetry.exporter.otlp.proto.http.trace_exporter.OTLPSpanExporter")
    def test_wrapper_instruments_fireworks_client(self, mock_exporter, mock_openai_response, mock_fireworks_client):
        """Should instrument Fireworks client calls."""
        from traceai_fireworks._wrappers import _CompletionsWrapper
        ...
        result = wrapper(
            ...
            (),
            {"model": "accounts/fireworks/models/llama-v3p1-8b-instruct", "messages": [{"role": "user", "content": "Hi"}]},
        )
