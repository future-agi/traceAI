...
def payload_to_semantic_attributes(
    ...
    if EventPayload.CHUNKS in payload and EventPayload.EMBEDDINGS in payload:
        attributes[EMBEDDING_EMBEDDINGS] = [
            ...
            for text, vector in zip(
                payload[EventPayload.CHUNKS], payload[EventPayload.EMBEDDINGS]
            )
        ]
    ...
    if response := (
        payload.get(EventPayload.RESPONSE) or payload.get(EventPayload.COMPLETION)
    ):
    ...
    if event_type is CBEventType.RERANKING:
        ...
        if nodes := payload.get(EventPayload.NODES):
            attributes[
                RERANKER_OUTPUT_DOCUMENTS if is_event_end else RERANKER_INPUT_DOCUMENTS
            ] = [
                {
                    DOCUMENT_ID: node_with_score.node.node_id,
                    DOCUMENT_SCORE: node_with_score.score,
...
class FiTraceCallbackHandler(BaseCallbackHandler):
    __slots__ = (
        "_tracer",
        "_lock",
        "_event_data",
    ...
    def __init__(self, tracer: trace_api.Tracer) -> None:
        ...
        self._tracer = tracer
        self._lock = RLock()
        self._event_data: Dict[_EventId, _EventData] = _BoundedDict(
            on_evict_fn=_finish_tracing
        )
        self._templating_parent_id: Dict[_EventId, _ParentId] = _BoundedDict()
        self._templating_payloads: Dict[_ParentId, List[Dict[str, Any]]] = (
            _BoundedDict()
        )
...
def _get_span_kind(event_type: Optional[CBEventType]) -> FiSpanKindValues:
    ...
    return _SPAN_KINDS.get(event_type, FiSpanKindValues.CHAIN)
...
def _get_response_output(response: Any) -> Iterator[Tuple[str, Any]]:
    ...
    if isinstance(response, ChatResponse):
        ...
        if content := message.content:
            ...
            yield OUTPUT_VALUE, safe_json_dumps(
                message.additional_kwargs, cls=_CustomJSONEncoder
            )
            yield OUTPUT_MIME_TYPE, FiMimeTypeValues.JSON.value
...
_SPAN_KINDS = MappingProxyType(
    {
        CBEventType.EMBEDDING: FiSpanKindValues.EMBEDDING,
        CBEventType.LLM: FiSpanKindValues.LLM,
        CBEventType.RETRIEVE: FiSpanKindValues.RETRIEVER,
        CBEventType.FUNCTION_CALL: FiSpanKindValues.TOOL,
        CBEventType.AGENT_STEP: FiSpanKindValues.AGENT,
        CBEventType.RERANKING: FiSpanKindValues.RERANKER,
    }
)
