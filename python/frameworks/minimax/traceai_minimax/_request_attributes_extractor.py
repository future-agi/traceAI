...
class _ChatCompletionRequestAttributesExtractor:
    """Extract span attributes from MiniMax chat completion request."""
    ...
    def get_extra_attributes_from_request(
        ...
        if "tools" in request_parameters:
            ...
            yield "minimax.tools_count", len(tools)
            for i, tool in enumerate(tools[:10]):  # Limit to first 10
                if "function" in tool:
                    func = tool["function"]
                    yield f"minimax.tools.{i}.name", func.get("name", "")
                    yield f"minimax.tools.{i}.description", func.get("description", "")[:200]  # Truncate

        # Response format
        if "response_format" in request_parameters:
            resp_format = request_parameters.get("response_format", {})
            if isinstance(resp_format, dict):
                yield "minimax.response_format", resp_format.get("type", "text")
