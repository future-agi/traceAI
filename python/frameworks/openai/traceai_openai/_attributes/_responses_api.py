...
class _ResponsesApiAttributes:
    ...
    @classmethod
    @stop_on_exception
    def _get_attributes_from_response_image_generation_call(
        cls,
        obj: responses.response_output_item.ImageGenerationCall,
        prefix: str = "",
    ) -> Iterator[Tuple[str, AttributeValue]]:
        if (image := obj.result) is not None:
            yield f"{prefix}{MessageContentAttributes.MESSAGE_CONTENT_TYPE}", "image"
            yield f"{prefix}{MessageContentAttributes.MESSAGE_CONTENT_IMAGE}.status", obj.status
            yield (
                f"{prefix}{MessageContentAttributes.MESSAGE_CONTENT_IMAGE}",
                image,
            )
    ...
    def _get_attributes_from_response_output_item(
        ...
        if obj.type == "message":
            ...
            yield f"{prefix}{MessageAttributes.MESSAGE_ROLE}", "assistant"
            yield from cls._get_attributes_from_response_image_generation_call(
                obj,
                f"{prefix}{MessageAttributes.MESSAGE_CONTENT}.0.",
            )
        elif obj.type == "code_interpreter_call":
            # TODO: Handle code interpreter call
            pass
