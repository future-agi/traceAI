...
from traceai_azure_openai._request_attributes_extractor import _RequestAttributesExtractor
from traceai_azure_openai._response_accumulator import (
    _ChatCompletionAccumulator,
    _CompletionAccumulator,
    _ResponsesAccumulator,
)
from traceai_azure_openai._response_attributes_extractor import _ResponseAttributesExtractor
from traceai_azure_openai._span_io_handler import add_io_to_span_attributes
from traceai_azure_openai._stream import _AsyncStream, _ResponseAccumulator, _Stream
from traceai_azure_openai._utils import (
    _as_input_attributes,
    _finish_tracing,
    _io_value_and_type,
    safe_json_dumps,
)
from traceai_azure_openai._with_span import _WithSpan
...
def _is_azure_openai_instance(instance: Any) -> bool:
    """Check if the client instance is an Azure OpenAI client."""
    try:
        from openai.lib.azure import AzureOpenAI, AsyncAzureOpenAI
        return isinstance(instance, (AzureOpenAI, AsyncAzureOpenAI))
    except ImportError:
        return False
...
class _WithOpenAI(ABC):
    ...
    def _get_attributes_from_instance(
        ...
        # Always set Azure as the provider for Azure OpenAI clients
        yield SpanAttributes.GEN_AI_PROVIDER_NAME, FiLLMProviderValues.AZURE.value
        # Extract Azure-specific attributes from the client instance
        if azure_deployment := getattr(instance, "_azure_deployment", None):
            yield "gen_ai.azure.deployment", azure_deployment
        if api_version := getattr(instance, "_api_version", None):
            yield "gen_ai.azure.api_version", api_version
        if (
            (base_url := getattr(instance, "base_url", None))
            and (host := getattr(base_url, "host", None))
            and isinstance(host, str)
        ):
            yield "server.address", host
...
class _Request(_WithTracer, _WithOpenAI):
    def __call__(
        ...
        # Only instrument Azure OpenAI client instances
        if not _is_azure_openai_instance(instance):
            return wrapped(*args, **kwargs)
        try:
